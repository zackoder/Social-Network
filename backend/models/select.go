...
func Get_followings_users(user_id, group_id int, host string) ([]utils.User, error) {
	Query := `
		...
		FROM
		    users u
		    INNER JOIN followers f ON f.followed_id = ?
		    AND f.follower_id = u.id
		    LEFT JOIN group_members gm ON gm.group_id = ?
		    AND gm.user_id = u.id
		    LEFT JOIN notifications n ON n.target_id = u.id
		    AND n.actor_id = ?
	...
	`
	rows, err := Db.Query(Query, user_id, group_id, group_id, user_id)
	if err != nil {
		return nil, err
	}
	var users []utils.User
